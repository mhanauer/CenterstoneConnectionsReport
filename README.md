...
##### Goal 5 Objective B ###### ##########################################
# Grabbing Goal 4 Objective C (employed); #### Goal 3 Objective C ######## and grab percentages and compare them to the state
# Get the difference score.  Then compare on eth white and nonwhite.

# Grab employed variable.  Then 1 and 2 are 1 for employed, 8, 9, and 10 are NA everything else is 0.
Goal5ObjectiveB = data.frame(GPRAAll$Employed.x, GPRAAll$Employed.y)
Goal5ObjectiveB = data.frame(apply(Goal5ObjectiveB , 2, function(x){ifelse(x == 1, 1, ifelse(x == 2, 1, ifelse(x == 8, NA, ifelse(x == 9, NA, ifelse(x == 10, NA, 0)))))}))
dim(Goal5ObjectiveB)
colnames(Goal5ObjectiveB)= c("Base", "Month6")
Goal5ObjectiveB = data.frame(Goal5ObjectiveB$Month6 - Goal5ObjectiveB$Base)
colnames(Goal5ObjectiveB) = c("Difference")

Goal5ObjectiveBEth = data.frame(GPRAAll$White.x)
Goal5ObjectiveBEth = data.frame(apply(Goal5ObjectiveBEth, 2, function(x){ifelse(x == 2, 0, ifelse(x == 3, NA, ifelse(x == 4, NA, x)))}))
Goal5ObjectiveB = data.frame(Goal5ObjectiveB,Goal5ObjectiveBEth)
Goal5ObjectiveB = data.frame(na.omit(Goal5ObjectiveB))
dim(Goal5ObjectiveB)
# Right now tracked two people who are nonwhite so need more data before statistical tests
