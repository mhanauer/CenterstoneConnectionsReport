#### Subset the data for time period
...
## So everything is matched from baseline to 6 months later. So we don't want people who have their reassessments after the quaterly date.
GPRAAll = subset(GPRAAll, InterviewDate.y < "2018-07-01")
# Your N cannot be bigger than this
dim(GPRAAll)
...
## Need to change the time frame here
# Look at the GPRA and see for baseline and 6 month client ID for baseline and 6-month and anyone higher than that for each will not be included in this analysis.
...
Goal3ObjectiveCBaseMonth6 = data.frame(ClientID = GPRAAll$ClientID,GPRAAll$InpatientAlcoholSANights.x, GPRAAll$InpatientAlcoholSANights.y, GPRAAll$OutpatientAlcoholSATimes.x, GPRAAll$OutpatientAlcoholSATimes.y, GPRAAll$ERAlcoholSATimes.x, GPRAAll$ERAlcoholSATimes.y)
...
Goal3ObjectiveDBaseMonth6 = data.frame(ClientID = GPRAAll$ClientID,GPRAAll$InpatientMentalNights.x, GPRAAll$InpatientMentalNights.y, GPRAAll$OutpatientMentalTimes.x, GPRAAll$OutpatientMentalTimes.y, GPRAAll$ERMentalTimes.x, GPRAAll$ERMentalTimes.y)
...
colnames(Goal3ObjectiveDBase) = c("Base")
...
colnames(Goal3ObjectiveDMonth6) = c("Month6")

Goal3ObjectiveDBaseMonth6 = data.frame(Goal3ObjectiveDMonth6, Goal3ObjectiveDBase)
colnames(Goal3ObjectiveDBaseMonth6) = c("Month6", "Base")
summary(Goal3ObjectiveDBaseMonth6)
...
# This gives the average number of benefits each person receives
mean(benefitsCon$apply.Goal3ObjectiveEBaseMonth6..1..sum..na.rm...TRUE., na.rm = TRUE)
...
#### Centerstone Goal 4 Hospitalizations just looking at the percentage of people who said yes to the follwing variables: InpatientPhysical, InpatientMental, InpatientAlcoholSA, ERPhysical, ERMental, ERAlcoholSA
### Getting the average percentage of people who experienced any of these events 
CenterstoneGoal4 = data.frame(GPRAAll$InpatientPhysical.x, GPRAAll$InpatientMental.x, GPRAAll$InpatientAlcoholSA.x, GPRAAll$ERPhysical.x, GPRAAll$ERMental.x, GPRAAll$ERAlcoholSA.x, GPRAAll$InpatientPhysical.y, GPRAAll$InpatientMental.y, GPRAAll$InpatientAlcoholSA.y, GPRAAll$ERPhysical.y, GPRAAll$ERMental.y, GPRAAll$ERAlcoholSA.y)
CenterstoneGoal4 = na.omit(CenterstoneGoal4)
head(CenterstoneGoal4)
dim(CenterstoneGoal4)

CenterstoneGoal4Base = data.frame(CenterstoneGoal4[,1:6])

write.csv(CenterstoneGoal4Base, "CenterstoneGoal4Base.csv", row.names = FALSE)
CenterstoneGoal4Base = read.csv("CenterstoneGoal4Base.csv", header = TRUE)
CenterstoneGoal4Base = rowSums(CenterstoneGoal4Base)
CenterstoneGoal4Base = ifelse(CenterstoneGoal4Base > 0, 1, 0)


CenterstoneGoal4Month6 = data.frame(CenterstoneGoal4[,7:12])
write.csv(CenterstoneGoal4Month6, "CenterstoneGoal4Month6.csv", row.names = FALSE)
CenterstoneGoal4Month6 = read.csv("CenterstoneGoal4Month6.csv", header = TRUE)
CenterstoneGoal4Month6 = rowSums(CenterstoneGoal4Month6)
CenterstoneGoal4Month6 = ifelse(CenterstoneGoal4Month6 > 0, 1, 0)


# Here for means
CenterstoneGoal4Base = mean(CenterstoneGoal4Base); CenterstoneGoal4Base
CenterstoneGoal4Month6 = mean(CenterstoneGoal4Month6); CenterstoneGoal4Month6
(CenterstoneGoal4Month6-CenterstoneGoal4Base)/CenterstoneGoal4Month6
...
Goal5ObjectiveBMil = data.frame(GPRAAll$MilitaryServed.x, GPRAAll$ActiveDuty.x, GPRAAll$Deployed.x, GPRAAll$FamilyDuty.x)
library(plyr)
# Count 2's and 3's
...
######################### Centerstone Metrics ################### ############################################################################
# 1. Goal3 Objective A for Substance Abuse
#2.  Goal3ObjectiveB = Project Objective 3 for Depression
#  3. ER Useage (see below)
## 4. Hospitalizations

## ER useage 
Goal3ObjectiveDBaseMonth6 = data.frame(ParticipantID = GPRAAll$ClientID, GPRAAll$ERMentalTimes.x, GPRAAll$ERMentalTimes.y, GPRAAll$ERAlcoholSATimes.x, GPRAAll$ERAlcoholSATimes.y)
# Get N here
dim(Goal3ObjectiveDBaseMonth6)
...
# 1. Goal 3 Objective A for Drugs 
# 2. Goal 3 Objective D
# 3. Goal 4 Objective C Increase employment readiness
# Goal 4 Housing
# 
# Using housing rate data 
(21-11)/21

### For getting the Average sessions per client, individual sessions, average group sessions
### Need to grab the discharge data 
## Don't include this one SvcOtherCaseMgmtSpec, SvcOtherModalitesSpec, SvcOtherEduSpec they have words
...
GPRAAllDischarge = subset(GPRAAll, InterviewType == 5)
# get N here
dim(GPRAAllDischarge)
## Now grab the discharge variables that may capture the outcomes
head(GPRAAllDischarge[,244:303])
GPRAAllDischarge = GPRAAllDischarge[,244:303]
GPRAAllDischarge$SvcOtherCaseMgmtSpec = NULL
GPRAAllDischarge$SvcOtherModalitesSpec = NULL
GPRAAllDischarge$SvcOtherEduSpec = NULL
# Make sure nothing funky is going on
summary(GPRAAllDischarge)
write.csv(GPRAAllDischarge, "GPRAAllDischarge.csv", row.names = FALSE)
GPRAAllDischarge = read.csv("GPRAAllDischarge.csv", header = TRUE)
summary(GPRAAllDischarge)
GPRAAllDischarge = colMeans(GPRAAllDischarge)
head(GPRAAllDischarge)
# Just stick with case management that seems to be the most relevant

### Goal 4 Housing

housing = data.frame(GPRAAll$LivingWhere.x, GPRAAll$LivingWhere.y)
housing = data.frame(apply(housing, 2, function(x){ifelse(x == 4, 1, 0)}))
